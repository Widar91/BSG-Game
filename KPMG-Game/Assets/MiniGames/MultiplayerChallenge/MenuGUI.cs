...
public class MenuGUI : MonoBehaviour {
	...
	void ChallengesWindow (int windowID){
		...
		List<MinigameChallenge> challenges = ServerConnector.getInstance().getMinigameChallenges();
	...
	void NewChallengeWindow (int windowID) {
		...
		List<String> players = ServerConnector.getInstance().getPlayersNames();
		...
		foreach (String p in players){
			...
			if(GUI.Button(new Rect(Screen.width*0.38f,(i+1)*30,160,25),"" + "Challenge")) {
				MinigameChallenge c = JSONConverter.getInstance().convertMinigameChallenge(ServerConnector.getInstance().newMinigameChallenge(p));
