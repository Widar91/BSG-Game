...
public class CityCamera : MonoBehaviour {
	...
	private float WorldTerrainPadding = 5f;

	//Camera Zoom
	public GameObject MainCamera;
	private GameObject ScrollAngle;
	private float maxCameraHeight = 75f;
	...
	void Start () {
		...
		print ("pos: " + WorldTerrain.transform.position.x);
		print ("size: " + WorldTerrain.terrainData.size.x);
		print ("padding: " + WorldTerrainPadding);
...
//		cameraLimits.BottomLimit = WorldTerrain.transform.position.z + WorldTerrainPadding;
		cameraLimits.LeftLimit   = WorldTerrain.transform.position.x + WorldTerrainPadding;
		cameraLimits.RightLimit  = WorldTerrain.transform.position.x + WorldTerrain.terrainData.size.x - WorldTerrainPadding;
		cameraLimits.TopLimit    = WorldTerrain.transform.position.z + WorldTerrain.terrainData.size.z - WorldTerrainPadding;
		cameraLimits.BottomLimit = WorldTerrain.transform.position.z + WorldTerrainPadding;

		print(cameraLimits.LeftLimit);
		print(cameraLimits.RightLimit);
		print(cameraLimits.BottomLimit);
		print(cameraLimits.TopLimit);
...
//		print ("passed z");
	...
	public bool isDesiredPositionOverBoundaries (Vector3 desiredTranslation) {
		...
		print ("trans: " + desiredTranslation);
		print ("desired: " + desiredWorldPosition);
		bool overBoundaries = false;
		//check boundaries
		if(desiredWorldPosition.x < cameraLimits.LeftLimit)
			overBoundaries = true;
		print ("left lim" + cameraLimits.LeftLimit);

		if(desiredWorldPosition.x > cameraLimits.RightLimit)
			overBoundaries = true;
		print ("right lim" + cameraLimits.RightLimit);	
