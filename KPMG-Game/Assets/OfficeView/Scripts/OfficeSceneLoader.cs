...
//	};
	...
	public void RemoveObject(string objectName, bool reload = true)
	{
		...
		// Update local cache
		this.loadedObjectNames.Remove(objectName);
	...
	public void AddObject(string objectName, bool reload = true)
	{
		...
		// Update local cache
		this.loadedObjectNames.Add(objectName);
	...
	{
		if (this.loadedObjectNames.Contains (objectname)) 
		{
	...
	public IEnumerable<Item> DesignScene()
	{
		...
		if ( this.GoingLive )
		{
			this.loadedObjectNames = ServerConnector.getInstance ().GetOfficeObjects ();
		}
//		else
//		{
...
//		}
		...
		if (loadedObjectNames.Count == 0)
		{
			...
			this.loadedObjectNames = new List<string>(this.defaultScene);		
		}
